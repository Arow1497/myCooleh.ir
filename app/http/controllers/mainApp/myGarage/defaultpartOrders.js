...
class GaragePartOrdersController extends Controller{
 // Private helper methods
    ...
    async #processAttachments(files, fileUploadPath, correlationType) {
        ...
        if (filename && fileUploadPath) {
            ...
            attachments.push({
                ...
                partOrderId: process.env.DEFAULT_PARTORDERSREQUESTS_ID,
            });
            } catch (error) {
    ...
    async getAllPartOrderReqs(req, res, next) {
        try {
        const orders = await prisma.garagePartOrder.findMany({
    ...
    async getOnePartOrderReqById(req, res, next) {
        ...
        const orders = await prisma.garagePartOrder.findUnique({
            where: { id: req.params.partOrderId },
            include: {
            publisher: true,
        ...
        if (!orders) {
            throw createError(HttpStatus.NOT_FOUND, "سفارش قطعه مورد نظر یافت نشد");
        }
    ...
    async removePartOrderReqById(req, res, next) {
        ...
        if (orders.publisherId !== req.user.id) {
            throw createError(HttpStatus.FORBIDDEN, "شما مجاز به حذف این سفارش قطعه نیستید");
        }

        await prisma.garagePartOrder.delete({
            where: { id: partOrderId }
        });
    ...
    async editPartOrderReqById(req, res, next) {
        ...
        if (orders.publisherId !== req.user.id) {
            throw createError(HttpStatus.FORBIDDEN, "شما مجاز به ویرایش این سفارش قطعه نیستید");
        }
        ...
        const updatedNotice = await prisma.garagePartOrder.update({
            where: { id: partOrderId },
            data: {
            ...updateData,
            attachments: {
    ...
    async toggleBookmark(req, res, next) {
        ...
        if (bookmark) {
            ...
            return res.status(HttpStatus.OK).json({
            statusCode: HttpStatus.OK,
            data: { message: "سفارش قطعه از علاقه‌مندی‌های شما حذف شد" }
            });
        }
        ...
        return res.status(HttpStatus.OK).json({
            statusCode: HttpStatus.OK,
            data: { message: "سفارش قطعه به علاقه‌مندی‌های شما اضافه شد" }
        });
        } catch (error) {
        next(error);
        }
    ...
    try {
        ...
        throw createError(HttpStatus.BAD_REQUEST, "این سفارش قطعه در حال حاضر قابل اشتراک‌گذاری نیست");
        }
    ...
    async addCoworkReqForPartOrderReqFromSupplierStore(req, res, next) {
        try {
            ...
            const ordersApprenticeAppReqs = await prismaModel.create({
            data: {
                garage: {
                connect: {
                    id: garageId,
            ...
            statusCode: HttpStatus.OK,
            data: {
                ordersApprenticeAppReqs
            }
            });
        } catch (error) {
            console.error("Error creating apprentice orders request:", error);
            next(error);
        }
    }
    ...
    async showSuplierStorePartOrderRequestsForRequesterGarage(req, res, next) {
        try {
            ...
            const requests = await prisma.garagePartOrder.findMany({
    ...
    async deleteThisSupplierStoreFromPartOrderRequest(req, res, next){
        try {
            ...
            const { partOrderId, apprenticeId } = params;

            const addApprentice = await prisma.garagePartOrder.update({
                where: {
                    id: partOrderId,
                },
                data: {
            ...
            console.error("Error showing all apprentice orders requests:", error);
            next(error)
        }
    }
    ...
    const ordersApprentice = await this.findApprenticeRequestById(partOrderId)
    // بررسی مالکیت پست توسط کاربر درخواست‌دهنده
    if (!ordersApprentice.apprenticeId.equals(apprenticeId)) {
        throw createError.NotAcceptable("ویرایش ترنزاکشتن فقط برای طرفین آن مجاز است");
    };

    const refuseRequest = await prisma.garagePartOrder.update({
        where: {
        id: partOrderId,
        },
        data: {
