...
const { ApprenticeRequestsModel } = require("../../../../models/Garages/projects");
...
class GarageAddApprenticeRequestController extends Controller{
   ...
   async createNewApprenticeRequest(req, res, next){
    try {
        ...
        const registrationDataBody = await projectSchema.validateAsync(req.body);
          ...
          const createApprenticeRequest = await ApprenticeRequestsModel.create({
            garageID,
            clientID,
            status: "accepted",
   ...
   async removeApprenticeRequestById(req, res, next){
    try {
        const {apprenticerequestID} = req.params;
        const applicantID = req.user._id;  // ایدی درخواست دهنده
    // پیدا کردن پست و اطمینان از موجودیت آن
    const apprenticeRequest = await this.findApprenticeRequestById(apprenticerequestID)
    ...
    if (!post.publisher.equals(applicantID)) {
        throw createError.NotAcceptable("ویرایش گفتمان فقط برای ناشر آن مجاز است");
           ...
           const removeProductResult = await ApprenticeRequestsModel.deleteOne({ _id: post._id });
   ...
   async editApprenticeRequestById(req, res, next) {
    try {
        await UpdateApprenticeRequestSchema.validateAsync(req.body);
        const { apprenticerequestID } = req.params;
        console.log(apprenticerequestID);
        const applicantID = req.user._id;
        // پیدا کردن پست و اطمینان از موجودیت آن
        const post = await this.findApprenticeRequestById(apprenticerequestID)
        // بررسی مالکیت پست توسط کاربر درخواست‌دهنده
        if (!post.publisher.equals(applicantID)) {
            throw createError.NotAcceptable("ویرایش گفتمان فقط برای ناشر آن مجاز است");
        }
        ...
        const updateApprenticeRequestResult = await ApprenticeRequestsModel.updateOne({ _id: post._id }, { $set: data });
        if (!updateApprenticeRequestResult.modifiedCount) {
        ...
        deleteFilesInPublicForApprenticeRequests(req.files);
        next(error);
    }
}
   ...
   async BookmarkDastyerRequest(req, res, next){
    try {
        ...
        const {apprenticerequestID} = req.parms;
        await this.findApprenticeRequestById(apprenticerequestID);
        let BookmarkedDastyerRequest = await ApprenticeRequestsModel.findOne({
            _id: apprenticerequestID,
            bookmarks : user._id
        })
        const updateQuery = BookmarkedDastyerRequest? {$pull:{bookmarks: user._id}} : {$push: {bookmarks: user._id}}
        await ApprenticeRequestsModel.updateOne({ _id: apprenticerequestID }, updateQuery)
        let message
   ...
   async LikeApprenticeRequest(req, res, next){
    try {
        const user = req.user._id;
        const {apprenticerequestID} = req.params;
        await findApprenticeRequestById(apprenticerequestID);
        let likedApprenticeRequest = await ApprenticeRequestsModel.findOne({
            _id: apprenticerequestID,
            likes : user._id
        })
        let disLikedApprenticeRequest = await ApprenticeRequestsModel.findOne({
            _id: apprenticerequestID,
            dislike : user._id
        })
        const updateQuery = likedApprenticeRequest? {$pull:{likes: user._id}} : {$push: {likes: user._id}}
        await ApprenticeRequestsModel.updateOne({ _id: apprenticerequestID }, updateQuery)
        let message
        if(!likedApprenticeRequest){
            if(disLikedApprenticeRequest) await ApprenticeRequestsModel.updateOne({ _id: apprenticerequestID }, {$pull: {dislike: user._id}})
   ...
   async dislikeApprenticeRequest(req, res, next){
    try {
        const user = req.user._id;
        const {apprenticerequestID} = req.params;
        await findApprenticeRequestById(apprenticerequestID)
        let likedpost = await ApprenticeRequestsModel.findOne({
            _id: apprenticerequestID,
            likes : user._id
        })
        let disLikedpost = await ApprenticeRequestsModel.findOne({
            _id: apprenticerequestID,
            dislikes : user._id
        })
        ...
        let message
        if(!disLikedpost){
            if(likedpost) await ApprenticeRequestsModel.updateOne({ _id: apprenticerequestID }, {$pull: {likes: user._id}})
   ...
   async getCommentsOfApprenticeRequest(req, res, next){
    try {
        const {apprenticerequestID} = req.params;
        await this.findApprenticeRequestById(apprenticerequestID);
        const post = await ApprenticeRequestsModel.findOne({_id: apprenticerequestID})
        .populate({
            path: "comments",
   ...
   async showApprenticesCoWorkingReqsForApprenticeRequest(req, res, next){
    try {
...
async addAndChooseApprenticeToApprenticeRequest(req, res, next){
    try {
...
async addApprenticeCommentAndRateForApprenticeRequest(req, res, next){
    try {
...
async addGarageCommentAndRateForApprenticeInApprenticeRequest(req, res, next){
    try {
...
async createInvoice_FactorForApprenticeRequest(req, res, next){
    try {
...
async shareApprenticeRequest(req, res, next){
   ...
   ///////////////////////////////////////////////////////////////////
   async findApprenticeRequestById(projectID) {
    const { id } = await ObjectIdValidator.validateAsync({ id: projectID });
    const project = await ApprenticeRequestsModel.findById(id);
