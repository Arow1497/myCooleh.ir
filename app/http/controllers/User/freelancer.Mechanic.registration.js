...
const { registrationSchema } = require("../../validators/admin/registration.schema");
   ...
   async mechanicRegistration(req, res, next){
    try {
      ...
      const image = ListOfImagesFromRequest(req?.files || [], req.body.fileUploadPath);
          ...
          const registration = await UsersModel.updateOne(
            {"_id": mechanicID},
            {$set: data},
            )
             // ایجاد اشتراک
          ...
          return res.status(201).json({
              statusCode: 201,
              data: {
                  message: " استادکار گرامی پروفایل شما با موفقیت بروز رسانی و اشتراک 3 ماهه رایگان برای شما فعال شد"
              }
          });
    } catch (error) {
        next(error)
