...
/*
و اینجا نحوه استفاده از این کانفیگ در routes:
...
// آپلود تک فایل
router.post('/upload/single', uploadSingleWithProcessing('file'), async (req, res) => {
  try {
    ...
    const media = await prisma.contentMedia.create({
      data: {
        ...
        userId: req.user.id, // باید از middleware authentication گرفته شود
...
// آپلود چند فایل
router.post('/upload/multiple', uploadMultipleWithProcessing('files'), async (req, res) => {
  try {
    const mediaPromises = req.processedFiles.map(file => 
...
uploadSingleWithProcessing: برای آپلود تک فایل
uploadMultipleWithProcessing: برای آپلود چند فایل
