...
اگر بخوام یکجا از این میدلور استفاده کنم و projectId رو ولیدیشن بکنم باید چطوری استفاده کنم؟


برای استفاده از میدلور برای ولیدیشن projectId، 
من دو روش رو نشون میدم - یکی برای روت‌های خاص و یکی برای استفاده عمومی در کل پروژه:

const express = require('express');
const publicValidators = require('./validators/publicValidators');

میدلور ولیدیشن برای پارامترهای URL
...
میدلور ولیدیشن برای بادی درخواست
const validateBody = (schema) => {
    return async (req, res, next) => {
        try {
            const validated = await schema.validateAsync(req.body);
            req.body = validated; // جایگزینی با دیتای ولید شده
            next();
        } catch (error) {
            return res.status(400).json({
                status: 'error',
...
اسکیمای ولیدیشن برای projectId
const projectIdSchema = publicValidators.getValidationSchema({
    projectId: publicValidators.id
});

روش اول: استفاده در یک روت خاص
...
روش دوم: استفاده در چندین روت با یک اسکیمای پیچیده‌تر
const createProjectSchema = publicValidators.getValidationSchema({
    title: publicValidators.title.required(),
    description: publicValidators.description,
    status: publicValidators.status,
    tags: publicValidators.tags
});

const updateProjectSchema = publicValidators.getValidationSchema({
    projectId: publicValidators.id.required(),
    title: publicValidators.title,
    description: publicValidators.description,
    status: publicValidators.status,
    tags: publicValidators.tags
});

مثال استفاده کامل در روت‌ها
...
روش سوم: ایجاد یک میدلور عمومی برای چک کردن projectId در همه روت‌ها
const validateProjectId = async (req, res, next) => {
    try {
        چک کردن در پارامترها
        if (req.params.projectId) {
            await projectIdSchema.validateAsync({ projectId: req.params.projectId });
        }
        چک کردن در کوئری
        if (req.query.projectId) {
            await projectIdSchema.validateAsync({ projectId: req.query.projectId });
        }
        چک کردن در بادی
        if (req.body.projectId) {
            await projectIdSchema.validateAsync({ projectId: req.body.projectId });
        }
        next();
    } catch (error) {
        return res.status(400).json({
...
استفاده عمومی در کل اپلیکیشن
app.use('/api/projects*', validateProjectId);

یا استفاده در یک روتر خاص
const projectRouter = express.Router();
projectRouter.use(validateProjectId);

مثال کامل یک روتر با همه موارد
const projectRouter = express.Router();

projectRouter.use(validateProjectId); // اعمال میدلور عمومی
...
ثبت روتر در اپلیکیشن
app.use('/api/projects', projectRouter);
نکات مهم:

میدلورهای validateParams و validateBody جداگانه ایجاد شدند چون معمولاً نیاز به رفتار متفاوتی دارند
میدلور عمومی validateProjectId می‌تونه projectId رو در هر جایی از درخواست چک کنه
اسکیماهای مختلف برای عملیات‌های مختلف (create, update) ایجاد شدند
خطاها به شکل یکپارچه مدیریت میشن
از prisma برای عملیات‌های دیتابیس استفاده شده
استفاده از این ساختار چند مزیت داره:

ولیدیشن یکپارچه در کل پروژه
جلوگیری از تکرار کد
مدیریت خطای بهتر
قابلیت توسعه آسان
*/