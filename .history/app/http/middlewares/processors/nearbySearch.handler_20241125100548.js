...
const generateCacheKey = (model, latitude, longitude, radius, filters) => {
  return `geo:${model}:${latitude}:${longitude}:${radius}:${JSON.stringify(filters)}`;
};

const invalidateModelCache = async (model) => {
  const pattern = `geo:${model}:*`;
...
// Main middleware factory
const createGeoFilterMiddleware2 = ({
  maxDistance = 25,
  latitudeField = 'latitude',
  longitudeField = 'longitude',
  prisma,
  ...
  // Create spatial indexes on startup
  const setupDatabaseIndexes = async () => {
    try {
      // Create spatial index for MariaDB
      await prisma.$executeRaw`
        CREATE SPATIAL INDEX IF NOT EXISTS idx_${model}_location 
        ON ${model} (POINT(${latitudeField}, ${longitudeField}))
      `;
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      // Try cache first
      if (cacheEnabled) {
        const cacheKey = generateCacheKey(model, userLatitude, userLongitude, radius, customWhere);
        try {
          ...
          logger.warn({
            message: 'Redis read error - continuing without cache',
            requestId,
            error: error.message
          });
        }
      ...
      // Use spatial query with MDEV-GIS
      const spatialQuery = `
        SELECT *, 
          ST_Distance_Sphere(
        ...
        ${Object.keys(customWhere).length > 0 ? 'AND ' + Object.entries(customWhere)
      ...
      const records = await prisma.$queryRaw(
        spatialQuery,
        [userLongitude, userLatitude, userLongitude, userLatitude, radius * 1000] // Convert km to meters
      );

      // Format records
      const filteredRecords = records.map(record => ({
        ...record,
        distance: Math.round(record.distance / 1000 * 100) / 100 // Convert to km and round to 2 decimals
      }));
...
module.exports = {
  createGeoFilterMiddleware2,
  invalidateModelCache
};

/*
تغییرات اصلی که در کد اعمال شده:

Database Indexing & Spatial Queries:

اضافه کردن تابع setupDatabaseIndexes که یک spatial index روی فیلدهای مکانی ایجاد می‌کند
استفاده از ST_Distance_Sphere برای محاسبه فاصله در MDEV-GIS
بهینه‌سازی کوئری‌ها با استفاده از توابع spatial


Cache Invalidation:

اضافه کردن تابع invalidateModelCache برای پاک کردن کش‌های مربوط به یک مدل خاص
بهبود مدیریت کلیدهای کش با تابع generateCacheKey
امکان invalid کردن کش از بیرون میدلور


Asynchronous Operations:

استفاده از promisify برای تبدیل توابع Redis به async/await
مدیریت بهتر خطاها در عملیات‌های async
بهبود error handling در عملیات‌های کش
...
مدیریت بهتر خطاهای Redis



برای استفاده از این میدلور بهبود یافته:
javascriptCopyconst { createGeoFilterMiddleware, invalidateModelCache } = require('./geoFilterMiddleware');

ایجاد میدلور
const geoFilter = createGeoFilterMiddleware({
  prisma,
  model: 'Product',
  maxDistance: 25,
...
اعمال روی روت
app.get('/products', geoFilter, (req, res) => {
  res.json(req.filteredRecords);
});

مثال برای invalid کردن کش بعد از تغییر در محصولات
