...
const buildErrorMetadata = (err, req, trackingId) => {
    ...
    return {
        ...
        errorLocation, // اضافه کردن مکان خطا
...
const logError = (err, req, metadata) => {
    const errorLog = {
        ...
        location: metadata.errorLocation
    };

    if (err.isOperational) {
        logger.error(`Operational Error at ${JSON.stringify(metadata.errorLocation)}`, errorLog);
    } else {
        logger.error(`Programming Error at ${JSON.stringify(metadata.errorLocation)}`, {
            ...errorLog,
            priority: 'high',
            needsImmediate: true
        });
...
const sendErrorDev = (err, req, res, metadata) => {
    ...
    return res.status(err.statusCode).json({
        ...
        message: err.message,
        stack: err.stack,
        location: metadata.errorLocation, // اضافه کردن مکان خطا به پاسخ
        ...
        switch (error.name) {
            ...
            case 'TypeError':
                logger.error(`TypeError occurred`, {
                    ...metadata,
                    stack: error.stack,
                    location: metadata.errorLocation, // اضافه کردن مکان خطا
...
const catchAsync = fn => {
    return (req, res, next) => {
        fn(req, res, next).catch(error => {
            const trackingId = generateTrackingId();
            const errorLocation = extractErrorLocation(error.stack);
...
// تابع جدید برای استخراج اطلاعات مکان خطا از stack trace
const extractErrorLocation = (stack) => {
    if (!stack) return 'Unknown location';
    
    try {
        // گرفتن اولین خط stack trace که معمولاً محل اصلی خطاست
        const stackLines = stack.split('\n');
        // حذف "Error:" از اول stack
        const errorLine = stackLines[1] || stackLines[0];
        // استخراج مسیر فایل و شماره خط
        const match = errorLine.match(/at\s+(?:.*\s+\()?(.+):(\d+):(\d+)/);
        
        if (match) {
            const [_, filePath, line, column] = match;
            // گرفتن نام فایل از مسیر کامل
        ...
        return 'Could not parse location';
    } catch (err) {
        return 'Error location parsing failed';
    }
};
