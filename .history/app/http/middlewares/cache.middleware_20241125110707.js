...
class CacheManager {
  ...
  async set(key, data, options = {}) {
    try {
      ...
      multi.set(key, compressed ? compressedData : JSON.stringify(cacheData), 'EX', ttl);
      multi.set(`${key}:metadata`, JSON.stringify(metadata), 'EX', ttl);

      // Store tag relationships
      for (const tag of tags) {
        multi.set(`${this.config.prefix}tag:${tag}:${key}`, '1', 'EX', ttl);
      }
    ...
    try {
      ...
      do {
        ...
        for (const key of keys) {
          ...
          keysToDelete.push(cacheKey, `${cacheKey}:metadata`, key);
        }
      } while (cursor !== '0');
