...
const handleUncaughtException = async (err) => {
  ...
  try {
      ...
      await new Promise((resolve, reject) => {
          ...
          // اطمینان از ثبت لاگ قبل از خروج
      ...
      // تاخیر کوتاه برای اطمینان از ثبت لاگ
      setTimeout(() => {
          process.exit(1);
      }, 1000);
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    switch (error.name) {
      case 'CastError':
        ...
        break;
        case 'UncaughtError':
          error = handleUncaughtException(error, metadata);
          break;
        case 'UnhandledError':
        error = handleUnhandledRejection(error, metadata);
        break;
