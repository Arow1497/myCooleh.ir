...
class ClanService {
    ...
    async createJoinRequest(userId, clanId, message) {
        // چک کردن عضویت قبلی
        const existingMembership = await this.prisma.clanMembership.findUnique({
        where: {
            clanId_userId: {
            clanId,
            userId,
        ...
        // چک کردن درخواست قبلی
        const existingRequest = await this.prisma.clanJoinRequest.findUnique({
        where: {
            clanId_userId: {
            clanId,
            userId,
    ...
    async approveJoinRequest(requestId, response) {
        ...
        // استفاده از تراکنش برای اطمینان از ثبت همه تغییرات
        return this.prisma.$transaction(async (tx) => {
        // بروزرسانی وضعیت درخواست
        const updatedRequest = await tx.clanJoinRequest.update({
            where: { id: requestId },
            data: {
            status: 'APPROVED',
            response,
        ...
        await tx.clanActivity.create({
            ...
            description: `New member joined the clan`,
            points: 10, // امتیاز پیش‌فرض برای عضو جدید
            },
        });
    ...
    async rejectJoinRequest(requestId, response) {
        const request = await this.prisma.clanJoinRequest.findUnique({
        where: { id: requestId },
        });
    ...
    async getPendingRequestsForClan(clanId, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        ...
        this.prisma.clanJoinRequest.count({
            where: {
            clanId,
            status: 'PENDING',
            },
    ...
    async getUserJoinRequests(userId, status = null) {
        const where = {
        userId,
        ...(status && { status }),
        };
