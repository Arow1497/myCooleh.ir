...
class GarageManagementController extends Controller {
    ...
    async #validateTransactionOwnership(transactionId, userId, role) {
        const transaction = await prisma.transaction.findUnique({
            ...
            include: {
                noticeApprentice: {
                    select: { apprenticeId: true, publisherId: true }
                }
            }
        });
    ...
    async createGarage(req) {
        try {
            ...
            return {
                ...
                data: {
                    message: "گاراژ با موفقیت ایجاد شد",
                    garage
                }
            };
        } catch (error) {
    ...
    async createOutsourcingRequest(req) {
        try {
            const garageId = await this.#validateGarageOwnership(req.user);
            const { projectId, description, requirements } = req.body;

            const outsourcingNotice = await prisma.noticeOutSourcing.create({
                data: {
                    projectId,
                    description,
                    requirements,
                    requesterId: garageId
                }
            });
            ...
            return {
                statusCode: HttpStatus.CREATED,
                data: {
                    message: "درخواست برون‌سپاری با موفقیت ایجاد شد",
                    notice: outsourcingNotice
                }
            };
        } catch (error) {
            throw createError.BadRequest(error.message);
    ...
    // Parts Management
    async createPartOrder(req) {
        try {
            const garageId = await this.#validateGarageOwnership(req.user);
            const { projectId, parts, supplierStoreId } = req.body;

            const garagePartOrder = await prisma.garagePartOrder.create({
                data: {
                    projectId,
                    garageId,
                    supplierStoreId,
            ...
            return {
                statusCode: HttpStatus.CREATED,
                data: {
                    message: "سفارش قطعات با موفقیت ثبت شد",
                    order: garagePartOrder
                }
            };
        } catch (error) {
            throw createError.BadRequest(error.message);
