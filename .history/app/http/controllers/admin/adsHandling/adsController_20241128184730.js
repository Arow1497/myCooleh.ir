...
class PlatformAdsController extends Controller {
    ...
    // New helper methods
    async #validateAdOwnership(adId, userId) {
        ...
        if (!ad) {
            throw createError(HttpStatus.FORBIDDEN, 'You do not have permission to update this advertisement');
        }
        return ad;
    }
  ...
  async #calculateAdMetrics(adId, startDate, endDate) {
   ...
   // New method for updating advertisements
  ...
  async pauseAd(req, res, next) {
   ...
   // New controller methods for admin and advanced functionality
   async approveAdByAdmin(req, res, next) {
    try {
        const { adId } = req.params;
        const { adminId, notes, validityPeriod } = req.body;
        ...
        if (!admin || admin.role !== 'ADMIN') {
            throw createError(HttpStatus.FORBIDDEN, 'Only administrators can approve ads');
        }

        const validUntil = new Date();
        validUntil.setDate(validUntil.getDate() + (validityPeriod || 30)); // Default 30 days
    ...
    async rejectAdByAdmin(req, res, next) {
        try {
            const { adId } = req.params;
            ...
            if (!admin || admin.role !== 'ADMIN') {
                throw createError(HttpStatus.FORBIDDEN, 'Only administrators can reject ads');
            }
            ...
            const approval = await prisma.adApproval.create({
                data: {
                    ...
                    validUntil: new Date() // Immediate expiration
                }
            });
    ...
    async processAdPayment(req, res, next) {
        try {
            const { adId } = req.params;
            ...
            if (ad.price !== amount) {
                throw createError(HttpStatus.BAD_REQUEST, 'Payment amount does not match ad price');
            }
    ...
    async updateAdTargeting(req, res, next) {
        try {
            const { adId } = req.params;
            const { targetAudience, displaySettings } = req.body;
    ...
    async getAdPerformanceReport(req, res, next) {
        try {
            const { adId } = req.params;
            const { startDate, endDate } = req.query;
    ...
    async extendAdDuration(req, res, next) {
        try {
            const { adId } = req.params;
            const { extensionDays, paymentId } = req.body;
            ...
            const updatedAd = await prisma.advertisement.update({
                ...
                data: {
                    endDate: newEndDate,
                    paymentId: paymentId || ad.paymentId
                }
            });
