    ...
    async closeConnections() {
        try {
            ...
            if (this.redisClient && this.redisClient.isOpen) {
                ...
                logger.info('Redis connection closed'); // استفاده از console.log به جای logger
            }
    
            // بستن اتصال Prisma
            await this.#prisma.$disconnect();
            logger.info('Prisma ORM disconnected.'); // استفاده از console.log به جای logger
    
            // در آخر بستن اتصال mongoose
            if (mongoose.connection.readyState === 1) {
                // آخرین لاگ قبل از بستن مونگو
                logger.info('Closing MongoDB connection...'); // استفاده از console.log به جای logger
                await mongoose.connection.close();
                logger.info('MongoDB connection closed'); // استفاده از console.log به جای logger
            } else {
            ...
            logger.info('Error closing database connections:', error?.message || error);
        }
    }
