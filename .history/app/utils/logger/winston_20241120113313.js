...
const createLogDirectories = () => {
  ...
  types.forEach(type => {
    ...
    // ایجاد زیرپوشه‌ها برای بخش custom
    if (type === 'custom') {
...
const getLogFileName = (type, section = null) => {
  const date = moment().format('YYYY-MM-DD');
  if (type === 'custom' && section) {
    return path.join(LOG_DIR, type, section, `${section}-${date}.log`);
  }
...
const createCustomSectionTransports = () => {
  return customSections.map(section => 
    new winston.transports.DailyRotateFile({
      filename: getLogFileName('custom', section),
      ...
      format: winston.format.combine(
        detailedFormat,
        createCustomFormat('custom', section)
      )
    })
  );
};
...
const totalTransports = 
  logTypes.length +         // ترنسپورت‌های اصلی
  customSections.length +   // ترنسپورت‌های سفارشی
  1;                       // ترنسپورت مونگو

console.log('Total number of transports:', totalTransports);
