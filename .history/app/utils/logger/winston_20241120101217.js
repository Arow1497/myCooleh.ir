...
const CUSTOM_LOG_DIR = path.join(LOG_DIR, 'custom');
  ...
  types.forEach(type => {
    ...
    if (type === 'custom') {
      customSections.forEach(section => {
        const sectionDir = path.join(typeDir, section);
...
class CustomSectionTransport extends winston.transports.DailyRotateFile {
  constructor(opts) {
    // یک مسیر پیش‌فرض برای filename تعیین می‌کنیم
    const defaultSection = customSections[0]; // استفاده از اولین سکشن به عنوان پیش‌فرض
    const defaultFilename = path.join(CUSTOM_LOG_DIR, defaultSection, `${defaultSection}-%DATE%.log`);
    
    super({
      ...opts,
      filename: defaultFilename // اضافه کردن filename به تنظیمات اولیه
    });
    
    this.name = 'CustomSectionTransport';
    this.sections = new Set(customSections);
  }

  log(info, callback) {
    const section = info.metadata?.section;
    if (info.metadata?.logType === 'custom' && this.sections.has(section)) {
      // به‌روزرسانی dirname و filename برای سکشن جاری
      const dirname = path.join(CUSTOM_LOG_DIR, section);
      const filename = `${section}-%DATE%.log`;
...
// ایجاد ترنسپورت مشترک برای همه بخش‌های custom
const customSectionTransport = new CustomSectionTransport({
  maxSize: '10m',
  maxFiles: '14d',
  datePattern: 'YYYY-MM-DD',
  zippedArchive: true,
  format: winston.format.combine(
    detailedFormat,
    winston.format((info) => {
      return info.metadata?.logType === 'custom' ? info : false;
    })()
  )
});
...
// ایجاد متدهای helper برای لاگ کردن در بخش‌های مختلف
customSections.forEach(section => {
