...
class EnhancedMongoTransport extends Transport {
  ...
  // متد برای ذخیره لاگ‌های معوق
  async savePendingLogs() {
      if (pendingLogs.length > 0 && mongoose.connection.readyState === 1) {
          try {
  ...
  async log(info, callback) {
      try {
          ...
          // اگر در حال خاموش شدن هستیم، لاگ را در حافظه نگه می‌داریم
          if (this.isShuttingDown) {
              pendingLogs.push(logEntry);
              return callback();
          }

          // اگر اتصال مونگو برقرار است، مستقیماً لاگ را ثبت می‌کنیم
          if (mongoose.connection.readyState === 1) {
              await this.collection.insertOne(logEntry);
          } else {
              // اگر اتصال برقرار نیست، لاگ را در حافظه نگه می‌داریم
              pendingLogs.push(logEntry);
          }
