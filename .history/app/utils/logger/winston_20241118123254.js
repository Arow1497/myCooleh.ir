...
function getCallerInfo() {
  ...
  // Skip first two lines (Error and getCallerInfo call)
...
const errorFormat = winston.format.printf(({ level, message, timestamp, stack, metadata, caller }) => {
  let output = `${timestamp} ${level}: ${message}`;
  
  if (caller) {
    output += `\nLocation: ${caller.file}:${caller.line} (${caller.function})`;
    output += `\nFull Path: ${caller.fullPath}`;
  }
...
const detailedFormat = winston.format.combine(
  ...
  winston.format.metadata({ fillExcept: ['message', 'level', 'timestamp', 'stack', 'caller'] }),
  winston.format.json(),
  errorFormat
);
...
// Override default logging methods to include caller information
['error', 'warn', 'info', 'http', 'debug'].forEach(level => {
  const originalMethod = logger[level].bind(logger);
  logger[level] = (...args) => {
    const callerInfo = getCallerInfo();
    if (typeof args[0] === 'object') {
      args[0].caller = callerInfo;
    } else {
      args.push({ caller: callerInfo });
    }
