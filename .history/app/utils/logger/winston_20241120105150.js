...
// Simplified log categories configuration
...
const createLogDirectories = () => {
  ...
  // Create directories for error logs
  fs.mkdirSync(path.join(LOG_DIR, 'error'), { recursive: true });
  
  // Create directory for monitoring logs
  fs.mkdirSync(path.join(LOG_DIR, 'monitoring'), { recursive: true });
  
  // Create directories for business logs
  ['custom', 'general'].forEach(type => {
...
// Logging levels and colors remain the same
const levels = {
  error: 0,
  warn: 1,
  info: 2,
...
// Error location extraction remains the same
...
// MongoDB Transport remains the same
...
// Optimized Transport Factory
function createRotateTransport(name, config, filterFn) {
  const baseConfig = {
    datePattern: 'YYYY-MM-DD',
    zippedArchive: true,
    ...config,
    filename: path.join(LOG_DIR, name, `${name}-%DATE%.log`),
  ...
  const transport = new winston.transports.DailyRotateFile(baseConfig);
  transport.name = name;
  return transport;
}
...
const createTransports = () => {
  ...
  // Standard error transport
  transports.push(createRotateTransport('error', LOG_CATEGORIES.standard.config,
    (info) => info.level === 'error'));

  // Combined monitoring transport
  transports.push(createRotateTransport('monitoring', LOG_CATEGORIES.monitoring.config,
    (info) => ['security', 'performance', 'system'].includes(info.metadata?.logType)));

  // Business transports
  ['custom', 'general'].forEach(type => {
    transports.push(createRotateTransport(type, LOG_CATEGORIES.business.config,
      (info) => info.metadata?.logType === type));
  });
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Log the number and names of transports
